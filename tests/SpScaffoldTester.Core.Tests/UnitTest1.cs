...
public class SqlFileScanServiceTests
{
    ...
    [Fact]
    public void RunScan_WithTopVariableAndSimpleAliasedColumns_ShouldParseResultColumns()
    {
        var tempFile = Path.Combine(Path.GetTempPath(), $"sp-scan-sql-{Guid.NewGuid():N}.sql");
        File.WriteAllText(
            tempFile,
    ...
    [Fact]
    public void RunScan_WithTopPercentAndSimpleAliasedColumns_ShouldParseResultColumns()
    {
        var tempFile = Path.Combine(Path.GetTempPath(), $"sp-scan-sql-{Guid.NewGuid():N}.sql");
        File.WriteAllText(
            tempFile,
