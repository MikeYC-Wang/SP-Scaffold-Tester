...
public class ScanCommandRunnerTests
{
    ...
    [Fact]
    public void Run_WithDistinctAndSimpleAliasedColumns_ShouldWriteResultColumns()
    {
        var tempSqlFile = Path.Combine(Path.GetTempPath(), $"sp-scan-sql-{Guid.NewGuid():N}.sql");
        var tempSnapshotFile = Path.Combine(Path.GetTempPath(), $"sp-scan-snapshot-{Guid.NewGuid():N}.json");
    ...
    [Fact]
    public void Run_WithTopVariableAndSimpleAliasedColumns_ShouldWriteResultColumns()
    {
        var tempSqlFile = Path.Combine(Path.GetTempPath(), $"sp-scan-sql-{Guid.NewGuid():N}.sql");
        var tempSnapshotFile = Path.Combine(Path.GetTempPath(), $"sp-scan-snapshot-{Guid.NewGuid():N}.json");
        ...
        File.WriteAllText(
            ...
            AS
            BEGIN
                SELECT TOP (@take) 1 AS id, NULL AS nickName;
            END
            """
        );
    ...
    [Fact]
    public void Run_WithTopPercentAndSimpleAliasedColumns_ShouldWriteResultColumns()
    {
        var tempSqlFile = Path.Combine(Path.GetTempPath(), $"sp-scan-sql-{Guid.NewGuid():N}.sql");
        var tempSnapshotFile = Path.Combine(Path.GetTempPath(), $"sp-scan-snapshot-{Guid.NewGuid():N}.json");
        ...
        File.WriteAllText(
            ...
            AS
            BEGIN
                SELECT TOP 10 PERCENT 1 AS id, NULL AS nickName;
            END
            """
        );
